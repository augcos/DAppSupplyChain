...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole{
  ...
  function harvestItem(uint _upc, address _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, 
    ...
    // Call modifier to check if caller is FarmerRole
    onlyFarmer()
    {
    ...
    items[_upc] = Item({
      ...
      originFarmerID: address(uint160(_originFarmerID)),
  ...
  function shipItem(uint _upc) public 
    // Call modifier to check if caller is DistributorRole
    onlyDistributor()
    // Call modifier to check if upc has passed previous supply chain stage
    sold(_upc)      
    // Call modifier to verify caller of this function
